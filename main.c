...
void cadastrar_prod_c() {
    ...
    if (choice == 1) {
        //Recebe informações do novo produto
        printf("| Nome do produto: ");
        fgets(nome, sizeof(nome), stdin);
        nome[strcspn(nome, "\n")] = 0;
        ...
        printf("| Validade (YYYY-MM-DD): ");
        fgets(validade, sizeof(validade), stdin);
        validade[strcspn(validade, "\n")] = 0; //remover o newline para que possa ter . e espaços
...
void cad_adm(){
    ...
    cJSON *cad_adm = cJSON_GetObjectItemCaseSensitive(root, "admins");
    if (!cJSON_IsArray(cad_adm)) {
        fprintf(stderr, "Formato de JSON inválido\n");
        cJSON_Delete(root);
        return;
    }
    ...
    int max_id = 0;
    cJSON *adm = NULL;
    cJSON_ArrayForEach(adm, cad_adm) {
        cJSON *json_id = cJSON_GetObjectItemCaseSensitive(adm, "id");
        if (cJSON_IsNumber(json_id) && json_id->valueint > max_id) {
            max_id = json_id->valueint;
        }
    }
    ...
    printf(" -------------------------------\n");
    printf("| Menu Controle de Gerência     |\n");
    printf("|-------------------------------|\n");
    printf("| Começar cadastro           - 1|\n");
    printf("| Visualizar Administradores - 2|\n");
    printf("| Atualizar Administrador    - 3|\n");
    printf("| Excluir Administrador      - 4|\n");
    printf("| Voltar - 5                    |\n");
    printf("|_______________________________|\n");
    printf("| Escolha sua opção: ");
    scanf("%d", &option);
    ...
    switch(option) {
        case 1:
            while (!cad_successful) {
                ...
                printf("| Insira o CPF (Somente números): ");
                scanf("%lld", &cpf);
                getchar();  //Consumir a nova linha caso tenha, garante que não vai separar por ponto nem por espaço os inputs
                ...
                bool adm_exists = false;
                cJSON_ArrayForEach(adm, cad_adm) {
                    cJSON *json_cpf = cJSON_GetObjectItemCaseSensitive(adm, "cpf");
                ...
                if (!adm_exists) {
                    cJSON *new_adm = cJSON_CreateObject();
                    cJSON_AddItemToObject(new_adm, "id", cJSON_CreateNumber(next_id)); // Adicionando ID
                    cJSON_AddItemToObject(new_adm, "username", cJSON_CreateString(username));
                    cJSON_AddItemToObject(new_adm, "cpf", cJSON_CreateNumber(cpf));
                    cJSON_AddItemToObject(new_adm, "email", cJSON_CreateString(email));
                    cJSON_AddItemToObject(new_adm, "password", cJSON_CreateString(password));
                    cJSON_AddItemToArray(cad_adm, new_adm);

                    data = cJSON_Print(root);
                    file = fopen("bd/adm_list.json", "w");
                    if (file) {
                        fwrite(data, 1, strlen(data), file);
                        fclose(file);
                        cad_successful = true;
...
void visualizar_adm(){
    ...
    cJSON *read_adm = cJSON_GetObjectItemCaseSensitive(root, "admin");
    if (!cJSON_IsArray(read_adm)) {
        fprintf(stderr, "Formato de JSON inválido\n");
        cJSON_Delete(root);
        return;
    }
    ...
    cJSON *adm = NULL;
    cJSON_ArrayForEach(adm, read_adm) {
        cJSON *id = cJSON_GetObjectItemCaseSensitive(adm, "id");
        cJSON *username = cJSON_GetObjectItemCaseSensitive(adm, "username");
        cJSON *cpf = cJSON_GetObjectItemCaseSensitive(adm, "cpf");
        cJSON *email = cJSON_GetObjectItemCaseSensitive(adm, "email");
...
void atualizar_adm(){
    system("cls || clear");
    int id;
    char new_username[50], new_email[100], password[100];
    long long new_cpf;
    bool user_found = false;
    ...
    cJSON *update_admin = cJSON_GetObjectItemCaseSensitive(root, "admins");
    if (!cJSON_IsArray(update_admin)) {
        fprintf(stderr, "Formato de JSON inválido\n");
        cJSON_Delete(root);
        return;
    }
    ...
    cJSON *adm = NULL;
    cJSON_ArrayForEach(adm, update_admin) {
        cJSON *json_id = cJSON_GetObjectItemCaseSensitive(adm, "id");
        if (cJSON_IsNumber(json_id) && json_id->valueint == id) {
            user_found = true;
            ...
            // Substituir os itens no objeto JSON
            cJSON_ReplaceItemInObject(adm, "username", cJSON_CreateString(new_username));
            cJSON_ReplaceItemInObject(adm, "cpf", cJSON_CreateNumber(new_cpf));
            cJSON_ReplaceItemInObject(adm, "email", cJSON_CreateString(new_email));
            cJSON_ReplaceItemInObject(adm, "password", cJSON_CreateString(password));
...
void excluir_adm(){
    ...
    cJSON *delete_adm = cJSON_GetObjectItemCaseSensitive(root, "admins");
    if (!cJSON_IsArray(delete_adm)) {
        fprintf(stderr, "Formato de JSON inválido\n");
        cJSON_Delete(root);
        return;
    }
    ...
    cJSON *adm = NULL;
    cJSON_ArrayForEach(adm, delete_adm) {
        cJSON *json_id = cJSON_GetObjectItemCaseSensitive(adm, "id");
        cJSON *username = cJSON_GetObjectItemCaseSensitive(adm, "username");
        if (cJSON_IsNumber(json_id) && json_id->valueint == id) {
            user_found = true;
            ...
            switch (option){
            case 1:
                cJSON_DeleteItemFromArray(delete_adm, cJSON_GetArraySize(delete_adm) - 1);
                printf(" ------------------------------ |\n");
                printf("| Usuário excluído com sucesso! |\n");
                printf("|-------------------------------|\n");
...
void dashboard_adm_navigate(){
    ...
    printf("| Controle de Adminstradores - 5|\n");
    printf("| Sair - 6                      |\n");
    printf("|-------------------------------|\n");
    printf("|Escolha uma opção: ");
    scanf("%d", &option);
